...
> AI-Log-Analyzer - Open source tool by AetherCodeHQ

`ai` `analysis` `cli` `golang` `logs` `open-source`

## What is it?

**Ai Log Analyzer** is a ai/ml tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/AI-Log-Analyzer.git
cd AI-Log-Analyzer
...
```bash
# Basic usage
./AI-Log-Analyzer --path ./target

# With options
./AI-Log-Analyzer --path ./target --format json --output report.json

# Verbose mode
./AI-Log-Analyzer --path ./target --verbose
```
...
### Basic scan
```bash
./AI-Log-Analyzer --path ./my-project
```

### JSON report
```bash
./AI-Log-Analyzer --path ./my-project --format json --output report.json
```
...
- name: Run Ai Log Analyzer
  run: ./AI-Log-Analyzer --path . --format json --output report.json
```
